...
RAG 파이프라인, 데이터 모델, 품질 관리 구조 요약. 항공 용어는 [README의 "도메인 30초" 표](README.md#비전공자를-위한-도메인-30초-항공-용어-없이-읽기) 참고.

---

## 1. 전체 그림

세 가지 사용자 흐름이 **하나의 RAG 인프라(embedding + pgvector)** 를 공유한다.
...
핵심 의도: **별도 시스템을 새로 짜지 않고 같은 파이프라인을 재사용**한다. "문제 생성"과 "출처 사전(FAA Reference)"은 동일한 임베딩·검색 위에 얹힌 두 표면이다.
...
**임베딩 차원 트레이드오프**: 3072차원을 쓴다. Supabase의 HNSW 인덱스는 2000차원 상한이라 이 임베딩은 인덱스 미사용(정확도 우선, 코퍼스 규모상 brute-force 검색으로 감내 가능한 범위). 의식적으로 받아들인 한계.
...
LLM 생성물의 품질을 한 군데가 아니라 세 단계에서 막는다.
...
2. **자동 스캔 (오프라인)** — `check_acs_quality.py`로 MCQ/Short/Oral 전수 패턴 스캔. C등급 자동 비활성화. 전수 검수 결과 C등급 비율 0.4%.
3. **운영 중 자동 정제 (적자생존)** — 사용자 👍 70%+ 문제는 풀 편입(`promote_to_pool`), 👎 30%+(최소 3명)는 비활성화(`blacklist`). 풀 5,000 초과 시 하위 점수 자동 탈락.
